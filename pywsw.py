...
class Connection(object): ##{{{
    ...
    def command(self, cmd, raw=False): ##{{{
        ...
        while retries > 0:
            self.send(cmd)
            try:
                if raw: response = self.receive(raw=True)
                else: response = self.receive_all()
            except Exception:
...
def test_connection(): ##{{{
    c = Connection("216.52.143.184", 44400)
    ...
    try:
        d = Connection("warsow-race.net", 44400)
        d.command("getstatus")
...
if __name__ == '__main__': ##{{{
    ...
    """
    servers = set()
    master = Connection( "dpmaster.deathmask.net", 27950, size=65536)
    data = master.command( "getservers Warsow %d %s" % (12, "full empty"), raw=True )
    d = ""
    for c in data:
        d += str((ord(c)))
    print d
